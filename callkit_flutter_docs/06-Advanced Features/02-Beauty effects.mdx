...
1. Open <code>Runner.xcworkspace</code> with Xcode.
2. Drag the downloaded <code>BeautyResources</code> folder to the Runner directory, and select <code>Create folder references</code>.
...
1. Create an assets folder under the <code>main</code> directory of your Android project &#40;no need to create one if the assets folder already exists&#41;, like this: **xxx/android/app/src/main/assets**
2. Copy the downloaded <code>BeautyResources</code> folder to the assets directory.
...
To set initial beauty effect, calling the <code>setBeautyParams</code> method of the <code>ZegoUIKitBeautyPlugin</code>.

Method API parameter description:

* <code>paramConfigList</code>：The list of beauty parameter configuration options you need to set.
* <code>forceUpdateCache</code>：If set to true, it will force the use of the current beauty configuration and clear previous cache records. if set to false, it will use the current configuration when there is no beauty cache record. If there is a beauty cache record, it will not use the current configuration.
...
To customize the beauty effects, config the <code>beautyConfig</code> in <code>ZegoUIKitPrebuiltCallConfig</code>.
...
In addition to the aforementioned function customization, you can also pass in a custom <code>effectsTypes</code> array to determine which beauty effect you want to use.
...
To customize the text related to the advanced beauty effects feature, you can use <code>ZegoBeautyPluginInnerText</code> like the following:
